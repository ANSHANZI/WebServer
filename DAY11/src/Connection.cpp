...
Connection::Connection(Eventloop *_loop, Socket *_sock)
    : loop(_loop), clnt_sockfd(_sock), channel(nullptr), readBuffer(nullptr),writeBuffer(nullptr) {
    
    channel = new Channel(loop,clnt_sockfd->getsockFd());
    channel->enableReading();
    channel->useET();

    std::function<void()> cb = std::bind(&Connection::echo, this, clnt_sockfd->getsockFd());
...
void Connection::echo(int clnt_sockfd) {
    while (true) {
        ...
        if (read_bytes > 0) {
            ...
            deleteConnectionCallback(clnt_sockfd); // 关闭socket会自动将文件描述符从epoll树上移除
            break;
            ...
            std::string echowords = "The server has received from client fd " + std::to_string(clnt_sockfd) + ": " + readBuffer->c_str() + "\n";
            // 使用 c_str() 获取字符串内容的 C 风格指针，使用 size() 获取实际长度
            writeBuffer->clear();
            writeBuffer->setBuf(echowords.c_str());
            size_t write_bytes = write(clnt_sockfd,  writeBuffer->c_str(),  writeBuffer->size());
