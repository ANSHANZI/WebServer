...
Server::Server(Eventloop* el):mainReactor(el){
    ...
    for(int i = 0; i < size; ++i){
        std::function<void()> sub_loop = std::bind(&Eventloop::loop, subReactors[i]);//将每个subReactor的loop函数绑定到sub_loop
...
void Server::newConnection(Socket *clnt_sockfd){
    if(clnt_sockfd->getsockFd() != -1){
        ...
        std::cout<<"new connection, fd: "<<clnt_sockfd->getsockFd()<<" bind to subReactor: "<<random<<std::endl;
        Connection *conn = new Connection(subReactors[random], clnt_sockfd);//随机选择一个Eventloop 与新连接绑定 处理新连接的事件
